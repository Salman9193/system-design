# System Design Prep — Google Staff Engineer (L6)

A structured preparation repo for the **system design rounds** of a Google Staff
Engineer (L6) loop. Covers both **HLD** (high-level / distributed systems design)
and **LLD** (low-level / object & API design), organized around what Google actually
evaluates at the staff level.
...
At L6, system design is **two rounds and carries the most weight** in the loop. The
most common reason for downleveling to L5 is giving an L5-quality design answer —
...
broken into the exact dimensions Google scores separately:

- **Requirements** — scoping and clarifying (10–15% of the rubric)
- **Design** — the high-level architecture (20–25%)
- **Deep Dives** — going inside 2–3 components (25–30%, the L5/L6 boundary)
...
│   └── STAFF_VS_SENIOR.md    what makes an answer L6, not L5
│
├── fundamentals/        The "inside the tools" knowledge Google probes
...
3. **Study the HLD/LLD templates** as models for how to think, not scripts to
   memorize. Google asks original/modified prompts, so pattern mastery beats recall.
...
L5 produces a correct design when asked. L6 **owns the system end-to-end** —
...
**Queued (HLD):** YouTube, Google Maps, Google Drive, URL shortener, Twitter
